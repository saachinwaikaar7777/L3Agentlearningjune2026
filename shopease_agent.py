...
# For the cart itself we key a plain dict by thread_id and use a
# contextvar to tell the tools which thread_id is "active" during the
# current agent.invoke() call. This mirrors how a real backend would
# scope a cart to a session/user id pulled from a request context.
# ---------------------------------------------------------------------------
...
def _cart_for_current_thread() -> dict:
    thread_id = _current_thread_id.get() or "__no_session__"
...
def add_to_cart(item_name: str, size: str = None) -> str:
    """Add an item (optionally with a size) to the current session's cart."""
    key = item_name.lower().strip()
    if key not in CATALOG:
        return f"Sorry, I couldn't find '{item_name}' in the catalog."
    cart = _cart_for_current_thread()
    cart["items"].append(
...
def invoke_agent(agent, thread_id: str, text: str, config: dict = None):
    """Helper: set the contextvar so tools know which cart to touch, then invoke."""
    token = _current_thread_id.set(thread_id)
    try:
        return agent.invoke({"messages": [HumanMessage(content=text)]}, config=config or {})
    finally:
...
def m1_prove_and_fix_bug():
    ...
    def chat(text: str):
        ...
        token = _current_thread_id.set(thread)
        try:
